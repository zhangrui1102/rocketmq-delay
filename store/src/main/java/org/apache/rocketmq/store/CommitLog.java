...
import org.apache.rocketmq.store.delay.exception.AppendException;
import org.apache.rocketmq.store.delay.startup.ServerWrapper;
import org.apache.rocketmq.store.delay.util.ConstantsUtils;
...
public class CommitLog {
    ...
    public CommitLog(final DefaultMessageStore defaultMessageStore,ServerWrapper serverWrapper) {
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        log.error("asyncPutMessage:{}",msg.toString());
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessages(final MessageExtBatch messageExtBatch) {
        log.error("asyncPutMessage:{}",messageExtBatch.toString());
        ...
        try {
            ...
            //新增延时队列
            appendDelayMessage(result,messageExtBatch);
            elapsedTimeInLock = this.defaultMessageStore.getSystemClock().now() - beginLockTimestamp;
            beginTimeInLock = 0;
        } finally {
            putMessageLock.unlock();
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        log.error("putMessage:{}",msg.toString());
        // Set the storage time
        msg.setStoreTimestamp(System.currentTimeMillis());
        // Set the message body BODY CRC (consider the most appropriate setting
        // on the client)
    ...
    public PutMessageResult putMessages(final MessageExtBatch messageExtBatch) {
        log.error("putMessage:{}",messageExtBatch.toString());
        messageExtBatch.setStoreTimestamp(System.currentTimeMillis());
        AppendMessageResult result;
    ...
    private void appendDelayMessage(AppendMessageResult result,MessageExt msgExt){
        log.error("start append delay message:{}",msgExt.toString());
        switch (result.getStatus()) {
            case PUT_OK:
                try {
                    if(msgExt.getDelayTimeLevel() > 0 && ConstantsUtils.getScheduleTime(msgExt.getProperties()) > 0){
                        serverWrapper.getFacade().appendScheduleLog(msgExt);
                    }
                } catch (Exception e) {
                    log.error("append schedule error:{}", e);
                    throw new AppendException(String.format("append schedule error,the reason %s",e.getMessage()));
                }
                break;
        }
    }
